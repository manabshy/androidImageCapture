...
$('select[name="vendorId"').on("change",function(){
	var seleVend = $(this).val();
	var uid = $('#username').text();
	sendVend = {vendId:seleVend};
	console.log('seleVend:%s',seleVend);
	$.ajax({
		        dataType: 'json',
		        data: sendVend,
		        type: 'POST',
...
$('.btn.btn-success.sendwoFormData').on('click',function(e){e.preventDefault() });
